...
if ($adminId) {
    ...
    mysqli_stmt_bind_param($updateStatusStmt, "i", $adminId);
    mysqli_stmt_execute($updateStatusStmt);
    mysqli_stmt_close($updateStatusStmt);

    // Use prepared statements to update user_logs record
    $updateLogQuery = "UPDATE user_logs SET logout_time = ?, status = ?, status_color = ?, status_symbol = ? WHERE user_id = ? AND logout_time IS NULL";
    $updateLogStmt = mysqli_prepare($conn, $updateLogQuery);

    if ($updateLogStmt) {
        mysqli_stmt_bind_param($updateLogStmt, "ssssi", $logoutTime, $status, $statusColor, $statusSymbol, $adminId);
